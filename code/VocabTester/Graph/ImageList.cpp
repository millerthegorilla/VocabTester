...
namespace ImageList
{
	...
	int Handle::AddImage(Win::Instance inst, int bitmapId )
	{
		HBITMAP hbm = ::LoadBitmap(inst.ToNative(), MAKEINTRESOURCE(bitmapId));
		int idx = ::ImageList_Add (ToNative(),hbm, NULL);
	...
	AutoHandle::AutoHandle (Win::Instance inst, int bitmapId, int imageWidth, Win::Color mask, int growCount)
		: Win::AutoHandle<ImageList::Handle>
		(::ImageList_LoadBitmap (inst, MAKEINTRESOURCE (bitmapId), imageWidth, growCount, mask))
	{
		if (IsNull ())
