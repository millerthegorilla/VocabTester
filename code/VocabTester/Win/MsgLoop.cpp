...
namespace Win
{
	...
	int MessagePrepro::Pump ()
	{
		...
		while ((status = ::GetMessage (&msg, 0, 0, 0 )) != 0)
		{
			...
			{			
				if (_hAccel.IsNull () 
					|| !::TranslateAccelerator (_hwndAccel.ToNative (), _hAccel.ToNative (), &msg))
				{
	...
	bool MessagePrepro::TranslateDialogMessage (MSG & msg)
	{
		...
		if (!_hDlgAccel.IsNull () 
			&& ::TranslateAccelerator (_hwndDlgAccel.ToNative (), _hDlgAccel.ToNative (), &msg))
		{
			return true;
		}
		return ::IsDialogMessage (_hwndDlg.ToNative (), &msg) != FALSE;
	}
	...
	int MessagePrepro::Pump (Win::Dow::Handle hwnd)
	{
		...
		while ((status = ::GetMessage (&msg, hwnd.ToNative (), 0, 0 )) != 0)
		{
			if (status == -1)
				throw Win::Exception ("Error in the Windows message loop");
			if (_hwndDlg.IsNull () || !::IsDialogMessage (_hwndDlg.ToNative (), &msg))
			{
				if (_hAccel.IsNull () 
					|| !::TranslateAccelerator (_hwndAccel.ToNative (), _hAccel.ToNative (), &msg))
				{
	...
	void MessagePrepro::PumpDialog ()
	{
		if (!_hwndDlg.IsNull ())
		{
			MSG  msg;
			int status;
			while ((status = ::PeekMessage (&msg, _hwndDlg.ToNative (), 0, 0, PM_REMOVE )) != 0)
			{
				if (status == -1)
					throw Win::Exception ("Error in the dialog peek message loop");
				if (!::IsDialogMessage (_hwndDlg.ToNative (), &msg))
				{
					::TranslateMessage (&msg);
					::DispatchMessage (&msg);
				}
