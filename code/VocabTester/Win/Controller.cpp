...
{
public:
	SameControl (Win::Dow::Handle winParent, int ctrlId)
...
void Win::Controller::RemoveDrawHandler (Win::Dow::Handle winParent, int ctrlId) throw ()
{
	_drawHandlers.remove_if (SameControl (winParent, ctrlId));
}

bool Win::Controller::OnItemDraw (OwnerDraw::Item & draw, int ctrlId) throw ()
{
	...
	case WM_RBUTTONDOWN:
		{
			POINTS p = MAKEPOINTS (lParam);
			KeyState kState (wParam);
			if (OnRButtonDown (p.x, p.y, kState))
				return true;
		}
		break;
	case WM_LBUTTONUP:
