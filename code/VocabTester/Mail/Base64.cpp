...
void Base64::Decoder::Decode (GenericInput<'\0'> & input)
{
	_buf.clear ();
	...
	for (;;)
	{
		...
		for (unsigned int i = 0; i < 4; ++i)
		{
			...
			if (c == '=')
			{
				if (i < 2)
				{
					throw "The base64 encoded input is corrupted.";
				}
				else if (i == 2)
				{
					c = input.Get ();
...
namespace UnitTest
{
	class Base64SimpleInput : public GenericInput<'\0'>
	{
	public:
		Base64SimpleInput (std::string const & src)
		: _it (src.begin ()), _end (src.end ())
		{
			if (src.size () < 4 ||	src.size () % 4 != 0)
				throw "The given base64 encoded data is corrupted.";
		}
		char Get ()
		{
	...
	void Base64Coding (unsigned int startLength, unsigned int finishLength)
	{
		for (unsigned int i = startLength; i < finishLength; ++i)
		{
			...
			Base64SimpleInput input (encoded);
			Base64::Decoder decoder;
			decoder.Decode (input);
			
			if (!std::equal (decoder.begin (), decoder.end (), src.begin ()))
