...
void _cdecl SET::ExceptionTranslator (unsigned int exCode, PEXCEPTION_POINTERS ep) throw (Win::ExitException)
{
		...
		exCode == VcppException (ERROR_SEVERITY_ERROR, ERROR_PROC_NOT_FOUND))
	{
		if (ep != 0)
		{
			...
			Msg info;
			info << "The required dynamic link library " << pdli->szDll;
			if (exCode == VcppException (ERROR_SEVERITY_ERROR, ERROR_MOD_NOT_FOUND))
			{
			...
			else
			{
				...
				if (pdli->dlp.fImportByName)
					...
					info << "with ordinal " << pdli->dlp.dwOrdinal;
			}
			info << ".";
			throw Win::ExitException ("Fatal error: missing or unuseable component.", info.c_str ()); 
		}
		throw Win::ExitException ("Fatal error: missing or unuseable Dynamic Link Library.");
	}
		...
		if (ep != 0)
		{
			...
			if (exRec != 0)
			{
				...
				if (exRec->ExceptionInformation [0] == 0)
					...
					info << "written.";
				throw Win::ExitException ("Fatal error: access violation.", info.c_str ());
			}
		}
	...
	throw Win::ExitException ("Fatal error: unrecoverable error occured during program execution.");
}
