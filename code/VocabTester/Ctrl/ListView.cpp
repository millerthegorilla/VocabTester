...
std::string ListView::RetrieveItemText (int pos, int subItem) const
{
	...
	item.PrepareForRead (pos, &buf [0], buf.length (), subItem);
...
void Report::AddColumn (int width, char const * title, ColAlignment align, bool hasImage)
{
	Report::Column column;
	column.Width (width);
	column.Align (align);
...
void Report::AddProportionalColumn (int widthPercentage, char const * title, ColAlignment align, bool hasImage)
{
    ClientRect rect (H ());
...
void Report::DeleteColumn (int iCol)
{
...
void Report::RemoveColumnImage (int iCol)
{
    Assert (iCol < _cColumn);
...
void Report::SetColumnImage (int iCol, int iImage, bool imageOnTheRight)
{
    Assert (iCol < _cColumn);
	Report::Column::Mask mask;
	mask.Format ();
...
void Report::SetColumnText (int iCol, char const * title, ColAlignment align, bool hasImage)
{
    Assert (iCol < _cColumn);
	Report::Column column;
	column.Align (align);
