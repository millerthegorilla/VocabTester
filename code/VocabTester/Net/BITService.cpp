...
namespace BITS
{
	...
	Com::IfacePtr<IEnumBackgroundCopyJobs> Service::GetJobs ()
	{
		...
		if (!SUCCEEDED (hr))
			throw Win::Exception ("Internal error: Cannot obtain Background Copy Job enumerator");
		...
		: _name (name), _jobIter (service.GetJobs ()), _totalJobCount (0), _curr (-1)
	{
		...
		if (!SUCCEEDED (hr))
			throw Win::Exception ("Internal error: Cannot obtain Background Copy Man job number");
	...
	void JobSeq::Advance ()
	{
		...
		while (!AtEnd ())
		{
			...
			if (!SUCCEEDED (hr))
				throw Win::Exception ("Internal error: Cannot advance Background Copy Job enumerator");
		...
		: _fileCount (0), _curr (-1)
	{
		...
		if (!SUCCEEDED (hr))
			throw Win::Exception ("Internal error: Cannot obtain Background Copy Files enumerator");
	...
	void FileSeq::Advance ()
	{
		...
		if (!SUCCEEDED (hr))
			throw Win::Exception ("Internal error: Cannot advance Background Copy File enumerator");
	}
