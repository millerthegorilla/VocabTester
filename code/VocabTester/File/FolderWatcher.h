...
{
public:
	FolderChangeEvent (std::string const & folder)
...
{
public:
    FolderWatcher (std::string const & dir, Win::Dow::Handle win)
        : FWatcher (win),
		  _folderEvent (dir)
    {}
private:
...
{
public:
	WatchedFolder (std::string const & folder)
		: _event (folder), _stopWatching (false)
	{}
	~WatchedFolder ()
	{
		if (_event.IsValid ())
