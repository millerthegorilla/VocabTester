...
public:
	...
	void DirDown (char const * subDir);
	void DirUp ();
	...
	char const * GetFilePath (char const * fileName) const;
	char const * GetFilePath (std::string const & fileName) const;
	...
	{
		return c == _backSlash || c == _forwardSlash;
	}
	static bool IsEqualDir (std::string dir1, std::string dir2);
	static bool IsDirLess (std::string dir1, std::string dir2);
...
protected:
	void Append (std::string const & subDir);
