	  ...
	  _refresh (true)
{
    ...
    {
		_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (*it)));
    }
}
...
void MultiFolderWatcher::AddFolder (std::string const & folder)
{
	//------------------------
	Win::Lock lock (_critSect);
	...
	if (it == _folders.end ())
	{
		...
		_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (folder)));
		_refresh = true;
...
void MultiFolderWatcher::AddFolders (std::vector<std::string> const & folders)
{
		 ...
		 ++newFolder)
	{
		...
		{
			// Folder not on the watch list -- add it
			_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (*newFolder)));
			_refresh = true;
		}
	}
	if (_refresh)
