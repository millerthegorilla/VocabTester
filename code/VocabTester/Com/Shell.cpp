...
void Path::Init (Com::IfacePtr<IShellFolder> & folder, char const * path)
{
	...
	HRESULT hresult = folder->ParseDisplayName (0, 0, &wPath [0], &lenParsed, &_p, 0);
	if (FAILED (hresult))
		throw Win::Exception ("Internal error: Cannot access shell folder", path);
}
							  ...
							  char const * dlgWinCaption)
{
    ...
    _browseInfo.lParam = reinterpret_cast<LPARAM>(dlgWinCaption);
...
int CALLBACK FolderBrowser::BrowseCallbackProc (HWND winBrowseDlg, UINT uMsg, LPARAM lParam, LPARAM lpData)
{
	if (lpData != 0)
	{
		// lpData == _browseInfo.lParam
		// Window caption specified -- display it
		Win::Dow::Handle dlgWin (winBrowseDlg);
		dlgWin.SetText (reinterpret_cast<char const *>(lpData));
	}
	return 0;
}
												...
												std::string const & verb)
{
	...
	HRESULT result = ::AssocQueryString (ASSOCF_NOTRUNCATE |	// Do not truncate the return string. Return an error value and the required size for the complete string.
	...
	if (result == E_POINTER)
	{
		// Buffer too small
		_cmd.resize (bufSize);
		result = ::AssocQueryString (ASSOCF_NOTRUNCATE |	// Do not truncate the return string. Return an error value and the required size for the complete string.
									 ASSOCF_REMAPRUNDLL,	// If a command uses Rundll.exe, setting this flag tells the method to ignore Rundll.exe and return information about its target.
									 ASSOCSTR_EXECUTABLE,	// Executable from a Shell verb command string.
									 fileNameExtension.c_str (),
									 verb.c_str (),
	...
	if (result != S_OK)
	{
		...
		throw Win::Exception ("Internal error: Cannot retrieve Shell verb command string", info.str ().c_str ());
	}
}
