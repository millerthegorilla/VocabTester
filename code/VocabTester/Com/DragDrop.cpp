...
FileDropPackage::FileDropPackage (Clipboard const & clipboard)
	: Handle<HDROP> (clipboard.GetFileDrop ())
{}

FileDropPackage::Sequencer::Sequencer (FileDropPackage const & fileDrop)
	: _handle (fileDrop),
	  _cur (-1)
{
...
void FileDropPackage::Sequencer::Advance ()
{
	Assert (!AtEnd ());
	_cur++;
	if (!AtEnd ())
...
std::ostream& operator<<(std::ostream& os, FORMATETC const * fmt)
{
	FormatEtc const * format = reinterpret_cast<FormatEtc const *>(fmt);
	os << "Format = (";
	switch (fmt->cfFormat)
	{
...
std::ostream& operator<<(std::ostream& os, STGMEDIUM const * stgMedium)
{
	StorageMedium const * medium = reinterpret_cast<StorageMedium const *>(stgMedium);
	os << "Storage Medium = ";
	if (medium->IsMemory ())
		os << "memory";
